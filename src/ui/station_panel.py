...
class StationPanel(QWidget):
    ...
    def _build(self) -> None:
        ...
        idx = next((i for i in range(self.retention_combo.count())
                    if self.retention_combo.itemData(i) == current), 0)
    ...
    def _retention_changed(self, _index: int) -> None:
        # Retention can change while the station is online; it takes effect on
        # the next background sweep, so persist immediately.
        self.config.retention_days = int(self.retention_combo.currentData() or 0)
        save_config(self.config)

    def set_remote_conflict(self, active: bool) -> None:
        """Called by the main window once it knows whether the cloud station
        worker is running. Both systems must never drive the same local job
        workspace at once, so Go Online is blocked while the cloud path owns
        it."""
        self._remote_active = active
        if active and self.station is None:
            self.toggle_button.setEnabled(False)
            self.toggle_button.setToolTip(
                "This PC is already online through your cloud account. Sign "
                "out to use the local-network-only mode instead.")
            self.status_label.setText(
                "Already online via your cloud account — the legacy local-"
                "network mode is disabled to avoid two render queues sharing "
                "the same folder.")
        elif not active:
    ...
    def _toggle_station(self) -> None:
        if self._remote_active and self.station is None:
            return
    ...
    def _refresh_jobs(self) -> None:
        ...
        for row, record in enumerate(records):
            name = QTableWidgetItem(
                f"{record.display_label} · {record.spec.name}"
                if record.spec.name else record.display_label)
